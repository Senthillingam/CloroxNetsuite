...
if(jsonobj.chl3!='' && jsonobj.chl3.length>0){
	nlapiLogExecution('DEBUG','chl3.length for   :: '+jsonobj.chl3.length);
	for(var cc = 0; cc < jsonobj.chl3.length ; cc ++){
		nlapiLogExecution('DEBUG','cc in for  :: '+jsonobj.chl3[cc]['internalid']);
...
var rptItems = [];
if(jsonobj.discounteditems!='' && jsonobj.discounteditems.length>0){
	nlapiLogExecution('DEBUG','discounteditems.length for rptItems   :: '+jsonobj.discounteditems.length);
	for(var cc = 0; cc < jsonobj.discounteditems.length ; cc ++){
		nlapiLogExecution('DEBUG','cc in for  :: '+jsonobj.discounteditems[cc]['internalid']);
