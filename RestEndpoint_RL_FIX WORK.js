    ...
    if (sRecType) {
        try {
            ...
            if (aResultSet) {
				...
				nlapiLogExecution('DEBUG', 'resultIndex', resultIndex);
				nlapiLogExecution('DEBUG', 'resultStep', resultStep);
				...
				// process or log the results
					...
					if(resultSet.length > 0)
					{
						...
						nlapiLogExecution('DEBUG', 'ResultSet', 'aResponse[i]length='+aResponse[i].length);
					}
					i++;
        // once no records are returned we already got all of them
				...
				if(newlength==1){
					nlapiLogExecution('DEBUG', 'Inside If for <1000:', '<1000');
